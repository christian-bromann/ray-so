/**
 * Theme CSS utilities for server-side Shiki theming.
 * Ported from app/(navigation)/(code)/util/theme-css-variables.ts
 */

/**
 * Options for creating a CSS variables theme
 */
...
/**
 * Shiki theme registration type (simplified for server-side use)
 */
...
/**
 * A factory function to create a css-variable-based theme for Shiki
 *
  ...
  if (!fontStyle) {
    theme.tokenColors = theme.tokenColors.map((tokenColor) => {
      if (tokenColor.settings?.fontStyle) {
        const { fontStyle, ...rest } = tokenColor.settings;
...
/**
 * Creates the default Shiki theme with ray.so CSS variable prefix
 */
export function createRayShikiTheme(): ThemeRegistration {
...
/**
 * Converts theme CSS properties to inline CSS string
 */
export function themeCSSToInlineStyle(cssProps: Record<string, string | undefined>): string {
...
/**
 * Generates CSS variables block from theme syntax object
 */
export function generateThemeCSSBlock(cssProps: Record<string, string | undefined>): string {
  const lines = Object.entries(cssProps)
    .filter(([, value]) => value !== undefined)
    .map(([key, value]) => `  ${key}: ${value};`);
