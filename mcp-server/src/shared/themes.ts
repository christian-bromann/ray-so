/**
 * Shared theme definitions for server-side usage.
 * Extracted from app/(navigation)/(code)/store/themes.ts
 * without Jotai atoms and browser-specific code.
 */

/**
 * Font type representing available font families
 */
export type Font =
  | "jetbrains-mono"
  | "fira-code"
...
/**
 * Shiki syntax highlighting object with CSS color values
 */
...
/**
 * CSS Properties object for theme styling
 */
export type ThemeCSSProperties = {
  "--ray-foreground"?: string;
  "--ray-token-constant"?: string;
  "--ray-token-string"?: string;
  "--ray-token-comment"?: string;
  "--ray-token-keyword"?: string;
  "--ray-token-parameter"?: string;
  "--ray-token-function"?: string;
  "--ray-token-string-expression"?: string;
  "--ray-token-punctuation"?: string;
  "--ray-token-link"?: string;
  "--ray-token-number"?: string;
  "--ray-token-property"?: string;
  "--ray-token-object-literal"?: string;
  "--ray-highlight"?: string;
  "--ray-highlight-border"?: string;
  "--ray-highlight-hover"?: string;
  "--ray-token-diff-deleted"?: string;
  "--ray-token-diff-inserted"?: string;
...
export type Theme = {
  ...
  lineNumbers?: boolean;
  syntax: {
    light?: ThemeCSSProperties;
    dark?: ThemeCSSProperties;
  };
};

/**
 * Converts a ShikiSyntaxObject to CSS custom properties for theming.
 */
export function convertToShikiTheme(syntaxObject: ShikiSyntaxObject | null | undefined): ThemeCSSProperties {
  if (!syntaxObject) {
    return {};
  }
...
/**
 * All available themes for code image generation.
 */
...
/**
 * Get theme IDs that are not hidden
 */
export function getVisibleThemeIds(): string[] {
  return Object.keys(THEMES).filter((id) => !THEMES[id].hidden);
}

/**
 * Get all theme IDs including hidden ones
 */
